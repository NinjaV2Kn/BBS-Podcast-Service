...
import { getYouTubeAuthUrl, exchangeCodeForToken, processEpisodeForYouTube } from '../utils/youtube';
 ...
 */
router.get('/callback', auth, async (req: Request, res: Response) => {
  try {
  ...
  try {
    ...
    (async () => {
      try {
        ...
        const youtubeId = await processEpisodeForYouTube(
          episode.audioUrl,
          episode.podcast.coverUrl,
          episode.title,
          episode.description || '',
          episode.podcast.title,
          [episode.podcast.title, 'podcast', 'audio']
        );
 ...
 */
router.delete('/disconnect', auth, async (req: Request, res: Response) => {
  try {
