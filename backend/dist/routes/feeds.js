...
function generateRssFeed(podcast, episodes, baseUrl) {
    ...
    <item>
      ...
      <enclosure url="${escapeXml((0, hash_1.normalizeUrl)(episode.audioUrl) || '')}" type="audio/mpeg" />
...
router.get('/all.xml', async (_req, res) => {
    try {
        ...
        const allEpisodes = podcasts
            .flatMap((podcast) => podcast.episodes.map((episode) => ({
            ...episode,
